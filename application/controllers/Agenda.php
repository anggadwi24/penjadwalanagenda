...
class Agenda extends CI_Controller 
{
	...
	public function index()
	{
		...
		if($this->role == 'admin' OR $this->role =='camat'){
			...
			if($status == 'sudah'){
				$where = 'WHERE agenda_validasi = "y" AND agenda_status != "done"';
			}else if($status == 'belum'){
				$where = 'WHERE agenda_validasi = "n" AND agenda_status != "done" ';
			...
			$data['record'] = $this->db->query("SELECT * FROM agenda WHERE agenda_validasi = 'y' AND agenda_validasi_by IS NOT NULL AND agenda_status != 'done'  $where ORDER BY agenda_id DESC");
			...
			if($this->role == 'admin'){
				...
				if($this->form_validation->run() == false){
					...
					if ($this->upload->do_upload('surat')){
						...
						$data = array('agenda_code'=>$kode,'agenda_name'=>$nama,'agenda_place'=>$tempat,'agenda_date_start'=>$dateStart,'agenda_date_end'=>$dateEnd,'agenda_penyelenggara'=>$penyelenggara,'agenda_penanggung_jawab'=>$pj,'agenda_perihal'=>$perihal,'agenda_by'=>$by,'agenda_kategori'=>$kategori,'agenda_validasi'=>'n','agenda_status'=>'undone','agenda_surat'=>$surat);
						if(count($staff) > 0){
